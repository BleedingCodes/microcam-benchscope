...
#!/usr/bin/env python3
"""
MicroCam BenchScope Pro

Python-only single-file HDMI microscope workstation for Linux USB capture cards
such as MacroSilicon HDMI-to-USB dongles.

Recommended install:

sudo apt install v4l-utils ffmpeg tesseract-ocr
python3 -m venv venv
...
APP_NAME = "MicroCam BenchScope Pro"
CONFIG_FILE = Path("microcam_benchscope_config.json")
...
def get_v4l2_controls(device: str) -> str:
    code, out, err = run_command(["v4l2-ctl", f"--device={device}", "--list-ctrls"])
    return out if out else err
...
def measurement_values(p1: Tuple[int, int], p2: Tuple[int, int]) -> Tuple[float, float, float]:
...
def draw_text(
    ...
    cv2.putText(frame, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, scale, (0, 0, 0), thick + 3)
...
class FrameProcessor:
    ...
    def process(raw: np.ndarray) -> np.ndarray:
        ...
        frame = FrameProcessor.apply_zoom_pan(frame)
        return frame
    ...
    def apply_zoom_pan(frame: np.ndarray) -> np.ndarray:
        ...
        center_x = (w - crop_w) // 2 + config.pan_x
        center_y = (h - crop_h) // 2 + config.pan_y

        x1 = max(0, min(center_x, max_x))
        y1 = max(0, min(center_y, max_y))
...
class CameraWorker(QObject):
    ...
    def open_capture(self) -> bool:
        ...
        self.status.emit(
            f"Opened {config.device}: requested {config.width}x{config.height}@{config.fps} "
            f"{config.fourcc}; actual {actual_w}x{actual_h}@{actual_fps:.1f}"
        )
        return True
...
class FFmpegRecorder:
    ...
    def start(self, width: int, height: int, fps: int) -> Path:
        ...
        cmd = [
            ...
            "libx264",
            "-preset",
            str(config.record_preset),
            "-crf",
            str(config.record_crf),
            "-pix_fmt",
            "yuv420p",
            str(self.file),
        ]
...
class VideoLabel(QLabel):
    ...
    def __init__(self):
        ...
        self.locked_measurements: List[Tuple[Tuple[int, int], Tuple[int, int], str]] = []
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            self.measurement_start = self.map_to_frame(event.position().x(), event.position().y())
            ...
            self.panning = True
            self.last_pan_pos = (event.position().x(), event.position().y())

    def mouseMoveEvent(self, event):
        if self.mouse_down:
            self.measurement_end = self.map_to_frame(event.position().x(), event.position().y())
            self.measurement_changed.emit()
    ...
    def lock_current_measurement(self):
        ...
        self.locked_measurements.append((self.measurement_start, self.measurement_end, label))
        self.measurement_changed.emit()
...
class MainWindow(QMainWindow):
    ...
    def make_image_tab(self):
        ...
        self.brightness_slider = self.make_slider(-100, 100, config.brightness, self.set_brightness)
        self.contrast_spin = self.make_double_spin(0.1, 5.0, 0.05, config.contrast, self.set_contrast)
        self.gamma_spin = self.make_double_spin(0.1, 5.0, 0.05, config.gamma, self.set_gamma)
        self.saturation_spin = self.make_double_spin(0.0, 3.0, 0.05, config.saturation, self.set_saturation)
        self.threshold_slider = self.make_slider(0, 255, config.threshold_value, self.set_threshold_value)
    ...
    def make_overlay_tab(self):
        ...
        self.zoom_spin = self.make_double_spin(1.0, 30.0, 0.25, config.zoom, self.set_zoom)
        layout.addWidget(QLabel("Zoom"), row, 0)
        layout.addWidget(self.zoom_spin, row, 1)
        row += 1

        self.known_mm_spin = self.make_double_spin(0.001, 10000.0, 0.1, config.known_mm, self.set_known_mm)
    ...
    def make_output_tab(self):
        ...
        rec_btn = QPushButton("Start / Stop Recording  R")
        ...
        self.snapshot_clean_cb.toggled.connect(lambda v: setattr(config, "snapshot_clean", v))

        self.recording_clean_cb = QCheckBox("Clean Recording")
        self.recording_clean_cb.setChecked(config.recording_clean)
        self.recording_clean_cb.toggled.connect(lambda v: setattr(config, "recording_clean", v))
        ...
        self.preset_box.currentTextChanged.connect(lambda v: setattr(config, "record_preset", v))
        ...
        layout.addWidget(QLabel("Recording CRF"), row, 0)
        layout.addWidget(self.crf_spin, row, 1)
        row += 1

        layout.addWidget(QLabel("x264 Preset"), row, 0)
    ...
    def set_hardware_control(self):
        ok, msg = set_v4l2_control(config.device, self.ctrl_name.text().strip(), self.ctrl_value.value())
    ...
    def set_known_mm(self, value):
        config.known_mm = value
    ...
    def update_display(self):
        ...
        self.last_tick = now
        instant_fps = 1.0 / dt
        self.ui_fps = instant_fps if self.ui_fps <= 0 else 0.9 * self.ui_fps + 0.1 * instant_fps
        ...
        if self.recorder.active:
            frame_to_record = self.processed_frame if config.recording_clean else self.display_frame
        ...
        if hasattr(self, "zoom_spin") and abs(self.zoom_spin.value() - config.zoom) > 0.001:
    ...
    def draw_overlays(self, frame: np.ndarray):
        ...
        if config.show_fps:
            rec_text = " REC" if self.recorder.active else ""
            draw_text(
                frame,
        ...
        if config.show_ruler:
            ...
            cv2.line(frame, (x0 + px_len, y0 - 10), (x0 + px_len, y0 + 10), (255, 255, 255), 2)
    ...
    def draw_measurement(
        ...
        x = max(10, min(p1[0], frame.shape[1] - 320))
        y = max(25, p1[1] - 12)
    ...
    def toggle_recording(self):
        if self.recorder.active:
            ...
            self.log(f"Recording stopped: {file}")
            return
        ...
        self.log(f"Recording started: {file}")
    ...
    def estimate_trace_width(self):
        ...
        for i, v in enumerate(bright_mask):
    ...
    def auto_white_balance_preview(self):
        ...
        gains = np.array([gray / avg_b, gray / avg_g, gray / avg_r], dtype=np.float32)
